...
# Stop the service
ssh -o StrictHostKeyChecking=no -p "$SSH_PASSWORD" "$SSH_USERNAME@$SSH_HOST" 'sudo systemctl daemon-reload; sudo systemctl stop gamedoora-user-services'
...
ssh -p "$SSH_PASSWORD" $SSH_USERNAME@$SSH_HOST 'sudo chown gamedoora:gamedoora /opt/gamedoora/\*.jar; sudo systemctl restart gamedoora-user-services'
